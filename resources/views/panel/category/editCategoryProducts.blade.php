        ...
        <section class="content">
            <div class="container-fluid ml-4">
                <div class="form_container">
                    <form action="{{route('category.updateCategoryProducts', $categoryIdea->id)}}" method="post" class="w-100" enctype="multipart/form-data">
                        @csrf
                        @method('patch')
                        ...
                        <div class="form_block row mb-3 ">
                            <div class="form-group col-lg-3 col-sm-6">
                                ...
                                <select class="custom-select form-control" id="category_id" name="category_idea_id" >
                                    @foreach($categoryIdeas as $item)
                                        <option {{$categoryIdea->id == $item->id ? 'selected' : ''}}  value="{{$item->id}}" >{{$item->name_ro}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-lg-3 col-sm-6">
