        ...
        <section class="content">
            <div class="container-fluid ml-4">
                <div class="form_container">
                    <form action="{{route('product.wheelUpdate',$wheel->id )}}" method="post" class="w-100" enctype="multipart/form-data">
                        @csrf
                        @method('patch')

                        @include("panel.components.forms.name",["title" => "Numele roții", "placeholder" => "Numele roții", "valueRo" => $wheel->name_ro, "valueRu" => $wheel->name_ru, "valueEn" => $wheel->name_en])

                        <div class="form_block row mb-3 ">
                            <div class="form-group col-lg-3 col-sm-6">
                                <label for="" class="">Ordinea</label>
                                <input type="number" class="form-control" name="sort_order" placeholder="Ordinea" value="{{$wheel->sort_order}}">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-6 ml-3 d-flex flex-wrap" id="previewImageContainer">
                                    @if($wheel->image)
                                        <a href="{{url('storage/'.$wheel->image)}}" data-fancybox data-caption="Single image">
                                            <img id="image" src="{{url('storage/'.$wheel->image)}}" alt="Preview Feature image" class="added_preview_image "  style="width: 300px">
                                        </a>
                                    @endif
                                </div>
                            </div>
